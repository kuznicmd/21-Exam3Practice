...
def run_test_integers():
    ...
    # Test 2:
    expected = [0, 4234, 7, 1, 1, 3, 4, 5, 6, 7, 8, 9, 7, -730, -2, 8, 9, 10]
    answer = integers([('a', 0, 1.76, 4234),
                       (7, 1, 'hello', 10.10),
                       [1, 2.5, 3, 4, 5, 6, 7, 8, 9],
                       ('hello', 'hi', 'bye'),
                       [],
                       ['oops', 7],
                       [[525, 1.5], [7, 4, 0, 8, 'i', 44]],
                       [-730, -2, 8, 9, 10],
                       [],
                       'hello'
                       ])
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'OATSSOOPSOSABCDEFPSBCDEFOSHICODS'
    answer = big_letters([(0, -3, 5),
                          'Ok whAt is ThiSS?',
                          (10, 'Ok what is ThiSSS?', 10, 'HELP'),
                          [],
                          ['oopS'],
                          'OOPS',
                          ['OOPS', 'HELP'],
        ...
        for j in range(len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k]) is str:
                for i in range(len(sequence_of_sequences[k][j])):
                    if sequence_of_sequences[k][j][i].isupper() is True:
                        string = string + '{}'.format(sequence_of_sequences[k][j][i])
